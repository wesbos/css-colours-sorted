...
export function complementaryColor(colorName) {
  // first find the hex for that color name
  const hex = colors[colorName];
  // first convert the color the hsl
  const [h, s, l] = hexToHSL(hex);
  // then return that hsl, but complimentary
  return [h + 180, `${s}%`, `${l}%`];
}
...
function countVowels(word) {
  let count = [...word].filter((letter, i) => ['a', 'e', 'i', 'o', 'u'].includes(letter)).length;
...
const colorsArray = Object.entries(colors).reduce((acc, [name, hex]) => {
  const colorObject = {
    name,
    hex,
    hsl: hexToHSL(hex),
...
export const colorsByLength = [...colorsArray].sort((a, b) => a.name.length - b.name.length);
export const colorsByHue = [...colorsArray].sort((a, b) => b.hsl[0] - a.hsl[0]);
export const colorsBySaturation = [...colorsArray].sort((a, b) => b.hsl[1] - a.hsl[1]);
